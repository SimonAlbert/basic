...
// update ndoe height (after setting up child nodes)
...
node_ptr insert(node_ptr t, T *v, int* lr_log) {
     ...
     *           B       C
     *        0/  \1   0/  \1
     * flag: 00   10   01  11 (由下向上)
     * situ: LL   LR   RL  RR
     */
    int flag = 0b11&*lr_log; // 00:LL 10:LR 01:RL 11:RR 只匹配最后两位就行, 最后两位是离自己最近的两层操作
    if(balanceFactor(t) > 1){ // 高度差大于1, 不平衡
        // 偏左
        if(flag == 0b00){ // LL
            rotate_right(t);
